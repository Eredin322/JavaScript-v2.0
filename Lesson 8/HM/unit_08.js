...
function t11() {
    ...
    let i = 0;
    while (i < elem.length){
        out.textContent += elem[i].textContent+ ' ';
        i++;
    }
}
...
function t12() {
    let array = document.querySelectorAll('.div-12');
    let a = 0;
    while (a < array.length){
        array[a].style.background = 'orange';
        a++; 
    }
}
...
function t14() {
    ...
    let put = document.querySelectorAll('.i-14');
    let a = 0;
    while (a < put.length){
        if(put[a].checked){
            out.textContent = put[a].value;
        }
        a++;
    }
}
