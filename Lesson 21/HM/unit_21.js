...
function t3(event) {
    if(event.changedTouches[0].target.classList[0] == 'div-3_1'){
...
function t8() {
    let rand = parseInt(0 - 0.5 + Math.random()*((a8.length - 1) - 0 + 1));
    console.log(rand);
...
function t9() {
    document.querySelector('.out-9').innerHTML = event.changedTouches.length;
}
...
function t11(event) {
    ...
    out.innerHTML = 'radiusX: ' + event.changedTouches[0].radiusX + '<br>' + 'radiusY: ' + event.changedTouches[0].radiusY;
}
